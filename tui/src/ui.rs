...
fn render_price_chart(frame: &mut Frame, app: &App, area: Rect, dimmed: bool) {
    ...
    let history = app.selected_price_history();

    if let Some(points) = history {
        if points.is_empty() {
            render_price_chart_empty(frame, area, block, text_color, "No price history available");
            return;
        }
